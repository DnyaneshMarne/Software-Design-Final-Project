...
class ObsDynamics():
    ...
    def __init__(self,Gamew):
        ...
        self.obstacle_x = 730
        self.facObj = Factory()
        self.batQueue = PriorityQueue()
        self.ghost = self.facObj.create("Ghost",self.obstacle_x,random.randint(100, 300))
        self.castle1 = self.facObj.create("Castle1",self.obstacle_x,random.randint(600, 600))
        self.castle2 = self.facObj.create("Castle2",self.obstacle_x,random.randint(600, 600))
        self.house = self.facObj.create("House",self.obstacle_x,random.randint(600, 600))
        self.tree = self.facObj.create("Tree",self.obstacle_x,random.randint(600, 600))
        self.pumpkin = self.facObj.create("Pumpkin",810,670) 
    ...
    def batObs(self):
        ...
        if self.batQueue.empty():
            self.clonned_ghost2 = self.facObj.clone(self.ghost)
            temp = self.gw.getCanvas().create_image(self.clonned_ghost2.x, self.clonned_ghost2.y, image=self.clonned_ghost2.get_obstacle())
            self.batQueue.put((self.clonned_ghost2.x, self.clonned_ghost2.y, random.randint(6,9), temp))
                
            self.gw.getWindow().after(self.gw.framerate,self.batMotion)


    def batMotion(self):
        if not self.batQueue.empty():
            bat_x , bat_y, bat_speed, bat_fig = self.batQueue.get()
            updated_x = bat_x - bat_speed
            if (updated_x < 150 and updated_x + 100 >= 55) and (self.gw.posY >= bat_y - 10 and self.gw.posY < bat_y + 10):
                self.gw.pause = True
                self.ScoreBoard()
                self.EndGameScreen()
            
            if updated_x >= -10 and self.gw.pause == False:
                self.gw.getCanvas().coords(bat_fig, updated_x,bat_y)
                self.batQueue.put((updated_x, bat_y, bat_speed, bat_fig))
                self.gw.getWindow().after(self.gw.framerate,self.batMotion)
            elif self.gw.pause == False:
                self.gw.getWindow().after(self.gw.framerate,self.batObs)
        else:
            self.gw.getWindow().after(self.gw.framerate,self.batObs)
